        ...
        <div class="col-md-4">
            <div class="card mb-4 box-shadow">
                <div class="card-body">
                    ...
                    <button id="${
                      item.playlistId
                    }" type="button" class="launch btn btn-primary" data-toggle="modal" data-target="#modal${
      item.playlistId
    }">
                        Ecouter
                    </button>
                    <a href="/authentification" id="vote${
                      item.playlistId
                    }"class="btn btn-success mt-2">J'aime</a>
                </div>
            </div>
        </div>   
        `;
    ...
    if (!arr.includes(item.playlistId)) {
      return ` 
        ...
        item.playlistId
                        ...
                        <form action="/voteforplaylist" method="post">
                            ...
                            <button id="vote${
                              item.playlistId
                            }" type="submit" class="btn btn-success mt-2">J'aime</button>
                        </form>
                    </div>
                </div>
            </div>
      ...
      return `
        ...
        item.playlistId
                        ...
                        <button id="vote${
                          item.playlistId
                        }" class="alreadyVoted btn btn-success mt-2">&#10003;</button>
                    </div>
                </div>
            </div>
            `;
            ...
            Array.from(launchPlaylistButtons).forEach(button => {
              button.addEventListener("click", () => {
                const playlistClicked = allPlaylists.filter(
                  playlist => playlist.playlistId === Number(button.id)
                );
    ...
    if (!username) {
      ...
      fetch("/connected")
        ...
        .then(allWildersWithPlaylists =>
          allWildersWithPlaylists.filter(wilder => wilder.pseudo === username)
        )
    ...
    form.addEventListener("submit", e => {
      ...
      fetch("/playlists", {
        ...
        .then(playlist => {
          const alertBox = document.getElementById("alert-box");
          alertBox.className = "alert alert-success";
          alertBox.innerHTML = `Votre playlist titre ${playlist.titre} (${
            playlist.id
          }) a bien été créée`;
        });
    });
